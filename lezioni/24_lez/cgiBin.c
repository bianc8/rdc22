...
int main()
{
   ...
   while (1) {
      ...
      if ( !strcmp(method,"GET")){
         // cgi-bin
         char command[100];
         filename = url+1; // prende anche il primo slash /index.html
         if (!strncmp(url, "/cgi-bin/", strlen("/cgi-bin/"))) {
               sprintf(command, "%s > tmpfile", url+strlen("/cgi-bin/")); // redirect shell to a tmpfile
               printf("Eseguo comando %s\n", command);
               system(command);                 // execute a shell command, return termination status of program
               strcpy(filename, "tmpfile");     // copy output to filename
         }
